...
void jumpSearch() {
    ...
    int step = sqrt(n);
    int prev = 0;
    merchandise *curr = head;

    while (curr != nullptr && curr->id < id) {
        prev += step;
        merchandise *prevNode = curr;
...
void boyermooreSearch() {
    ...
    cout << "\nHasil pencarian untuk '" << target << "':\n";
    cout << "----------------------------------------------\n";

    while (temp != nullptr) {
        if (temp->nama.find(target) != string::npos) {
            cout << "ID      : " << temp->id << endl;
            cout << "Tipe    : " << temp->tipe << endl;
            cout << "Nama    : " << temp->nama << endl;
            cout << "Harga   : Rp." << temp->harga << endl;
...
void search() {
    ...
    cout << "Pilih jenis pencarian (1/2/3): ";
    cin >> pilihan;
    cin.ignore();
